        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              className="bg-[#1A1D1F] p-6 rounded-xl backdrop-blur-lg border border-white/5 hover:border-[#00A3FF]/20 transition-colors"
            >
              <div className="flex items-start gap-4 mb-4">
                <img
                  src={testimonial.avatar}
                  alt={testimonial.name}
                  className="w-12 h-12 rounded-full bg-[#00A3FF]/10"
                />
                <div>
                  <h3 className="font-semibold text-white">{testimonial.name}</h3>
                  <div className="flex items-center gap-1 mt-1">
                    {Array.from({ length: testimonial.rating }).map((_, i) => (
                      <Icons.Star key={i} className="w-4 h-4 text-[#00A3FF]" />
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-sm text-white/40">
                ...
                <span className="text-[#00A3FF]">{testimonial.purchase}</span>
              </div>
            </motion.div>
          ))}
        </div>
